...
func TestGetSyncFetchesAndParses(t *testing.T) {
	var gotPath, gotToken, gotDevice string
	...
	cache := stats.NewSpotifyCache(srv.URL, "tok123")
	...
	if gotPath != "/stats/spotify" || gotToken != "tok123" || gotDevice != "dev1" {
		t.Fatalf("request mismatch: path=%s token=%s device=%s", gotPath, gotToken, gotDevice)
	}
}

func TestGetEmptyDeviceReturnsNil(t *testing.T) {
	cache := stats.NewSummaryCache("http://example.invalid", "tok", "day")
	if cache.Get("") != nil {
